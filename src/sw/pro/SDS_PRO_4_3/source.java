...
class source {
	static int[]				distance;
	static LinkedList<Integer>	closed	= new LinkedList<>();
	static int[]				opened;
	static int[][]				map;
	static int					N;
	...
	static void Dijkstra(final int begin, final int visitor) {
		...
		do {
			...
			for (Integer from : closed) {
				for (int i = 1; i <= N; i++) {
					if (map[from.intValue()][i] > 0 && opened[i] != visitor) {
						if (sumDistance == 0 || sumDistance > distance[from.intValue()] + map[from.intValue()][i]) {
							sumDistance = distance[from.intValue()] + map[from.intValue()][i];
							next = i;
						}
					}
				}
